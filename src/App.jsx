...
const STEPS = [
  { id:"sexe", title:"Quel genre recherchez-vous ?", icon:Heart, category:"Base", options:[{value:"femme",label:"Une femme",icon:Heart},{value:"homme",label:"Un homme",icon:UserCircle}] },
...
function calculateCandidates(answers) {
  ...
  if (answers.sexe) { const f=DATA.sexe[answers.sexe]; pool*=f; factors.push({name:"Genre",factor:f}); }
  ...
  if (answers.taille_min!=null&&answers.taille_max!=null) { const st=answers.sexe==="homme"?DATA.taille_homme:DATA.taille_femme; const f=gaussianRangeProb(answers.taille_min,answers.taille_max,st.mean,st.std); pool*=f; factors.push({name:"Taille",factor:f}); }
...
export default function App() {
  ...
  const tailleStats = answers.sexe==="homme"?DATA.taille_homme:DATA.taille_femme;
        ...
        {phase==="intro"&&(
          <div className="text-center max-w-lg slide-up">
            ...
            <div className="mt-10 flex items-center justify-center gap-6 text-xs" style={{color:"rgba(255,255,255,0.25)"}}>
              ...
              <span className="flex items-center gap-1"><Search size={12}/> 13 critères</span>
